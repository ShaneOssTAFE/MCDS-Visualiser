...
function initGraph(nodes, links, schema) {
  ...
  // Fix for cameraDistance not being a function
  window.addEventListener('wheel', e => {
    const camera = Graph.camera();
    const target = Graph.cameraPosition().lookAt || { x: 0, y: 0, z: 0 };
    const currentDistance = camera.position.distanceTo(new THREE.Vector3(target.x, target.y, target.z));
    const newDistance = currentDistance * (e.deltaY > 0 ? 1.1 : 0.9);
    const clampedDistance = Math.max(200, Math.min(2000, newDistance));
    camera.position.set(
      target.x + (camera.position.x - target.x) * (clampedDistance / currentDistance),
      target.y + (camera.position.y - target.y) * (clampedDistance / currentDistance),
      target.z + (camera.position.z - target.z) * (clampedDistance / currentDistance)
    );
    Graph.cameraPosition({ x: camera.position.x, y: camera.position.y, z: camera.position.z }, target, 200);
  });
